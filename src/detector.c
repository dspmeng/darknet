...
void infer_image(network net, const char* input, char** names, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    int j;
    float nms=.4;
...
void infer_video(network net, const char* input, char** names, float thresh, float hier_thresh, char *outfile, int fullscreen, int mosaic)
{
    ...
    while (src = cvQueryFrame(capture)) {
        ...
        if (outfile && writer == NULL) {
            writer = cvCreateVideoWriter(outfile, CV_FOURCC('X','2','6','4'), 25.0, size, 1);
        }
